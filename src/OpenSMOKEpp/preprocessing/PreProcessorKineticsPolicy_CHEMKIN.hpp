...
namespace OpenSMOKE
{
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::ReadFromASCIIFile(const std::string file_name)
	{
...

		{
			...
			// Polimi Soot Discrete Sectional Method
			polimi_soot_classes_ = new PolimiSootClasses(myKinetics->soot_class_names(), myKinetics->soot_class_lines_abs());
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::KineticsFromASCIIFile(AtomicCompositionTable& atomicComposition, std::ostream& flog)
	{
		...
		// Polimi soot (if active)
		if (polimi_soot_classes_->is_active() == true)
		{
			for (unsigned int j = 0; j < number_of_reactions; j++)
				polimi_soot_classes_->filter_reaction(j, myKinetics->indices_of_good_lines()[reaction_lines[j]]);
		}
		...
		{
			Eigen::MatrixXi duplicate_reactions;
							...
							if ( reactions_[i].Tag() == reactions_[j].Tag() )
							{
								if (reactions_[i].IsDuplicate() == false || reactions_[j].IsDuplicate() == false )
								{
									if (reactions_[i].pressureDependentSpeciesIndex() == reactions_[j].pressureDependentSpeciesIndex())
									{
										...
										data(0) = i+1; data(1) = myKinetics->indices_of_good_lines()[reaction_lines[i]]; 
										data(2) = j+1; data(3) = myKinetics->indices_of_good_lines()[reaction_lines[j]]; 
										duplicate_reactions.conservativeResize(duplicate_reactions.rows()+1, 4);
										duplicate_reactions.row(duplicate_reactions.rows()-1) = data;
									}
								}
							}
						}
			...
			if (duplicate_reactions.rows() != 0)
			{
				std::cout << "Preprocessing failure because of " << duplicate_reactions.rows() << " undeclared DUPLICATE reaction(s)" << std::endl;
				std::cout << "Details are reported in the log file in the output folder" << std::endl;
 
				flog << "---------------------------------------------------------------------------------" << std::endl;
				flog << "FATAL ERROR: undeclared DUPLICATE reactions" << std::endl;	
				flog << "---------------------------------------------------------------------------------" << std::endl;
				flog << "Reaction    Line        Reaction    Line        " << std::endl;
				flog << "---------------------------------------------------------------------------------" << std::endl;
				for (unsigned int i=0;i<duplicate_reactions.rows();i++)
				{
					flog << std::setw(12) << std::left << duplicate_reactions(i,0);
					flog << std::setw(12) << std::left << duplicate_reactions(i,1);
					flog << std::setw(12) << std::left << duplicate_reactions(i,2);
					flog << std::setw(12) << std::left << duplicate_reactions(i,3);
					flog << std::endl;
				}
				flog << "---------------------------------------------------------------------------------" << std::endl;
				flog << std::endl;
	...
	template<typename Reactions>
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::KineticsToXMLCHEMKINFile(std::ostream& fOut)
	{
		// Reaction lines
		{
			fOut << "<reaction-chemkin>" << std::endl;
			for (unsigned int j = 0; j < reactions_.size(); j++)
			{
				std::stringstream reaction_data;
				reactions_[j].GetReactionStringCHEMKIN(names_species(), reaction_data, myKinetics->strong_comments()[reaction_lines[j]]);

				const unsigned int number_of_lines = CountSubStringOccurrences(reaction_data.str(), "\n");
				fOut << "<reaction i=\"" << j << "\" lines=\"" << number_of_lines << "\">" << std::endl;
				fOut << reaction_data.str();
		...
		// Stoichiometric matrix
		{
			...
			for (unsigned int j = 0; j < reactions_.size(); j++)
				count += reactions_[j].reactant_nu_indices().size() + reactions_[j].product_nu_indices().size();
			
			fOut << "<stoichiometric-matrix nz=\"" << count << "\">" << std::endl;
			for (unsigned int j = 0; j < reactions_.size(); j++)
			{
				fOut << "<reaction i=\"" << j
					<< "\" nr=\"" << reactions_[j].reactant_nu_indices().size()
					<< "\" np=\"" << reactions_[j].product_nu_indices().size() << "\">" << std::endl;

				for (unsigned int i = 0; i < reactions_[j].reactant_nu_indices().size(); i++)
					fOut << reactions_[j].reactant_nu_indices()[i] << " " << reactions_[j].reactant_nu()[i] << std::endl;
				for (unsigned int i = 0; i < reactions_[j].product_nu_indices().size(); i++)
					fOut << reactions_[j].product_nu_indices()[i] << " " << reactions_[j].product_nu()[i] << std::endl;
				
				fOut << "</reaction>" << std::endl;
			}
			fOut << "</stoichiometric-matrix>" << std::endl;
		}
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::WriteXMLFile(std::stringstream& fOutput) const
	{
		...
		if (polimi_soot_classes_->is_active() == true)
		{
			polimi_soot_classes_->WriteXMLFile(fOutput);
			polimi_soot_classes_->Summary(std::cout);
		}
