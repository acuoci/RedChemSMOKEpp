...
//		Omega11jk(T);
	...
	bool CheckFittingCoefficients(const Eigen::MatrixXd& fitting_coeffs)
	{
		for(int i=0;i<fitting_coeffs.rows(); i++)
			for(int j=0;j<fitting_coeffs.cols(); j++)
				if (std::isnan(fitting_coeffs(i,j)) == true || std::isinf(fitting_coeffs(i,j)) == true)
					return false;
		return true;
	}
	...
	{
		bool is_eta_feasible = false;
		bool is_lambda_feasible = false;
		bool is_gamma_feasible = false;
		bool is_teta_feasible = false;
		...
		for (int k=0;k<10;k++)
		{
			...
			std::cout << " * Fitting transport properties from " << TMIN << "K to " << TMAX << "K in " << nPoints << " points..." << std::endl;
	
			// Fitting Viscosity
			if (is_eta_feasible == false)
			{
				...
				// Check if the coefficients have been properly calculated
				if (CheckFittingCoefficients(fittingEta) == false)
					std::cout << "    - WARNING: unable to fit the viscosity..." << std::endl;
				else
					is_eta_feasible = true;
			}

			// Fitting Conductivities
			if (is_lambda_feasible == false)
			{
				...
				if (CheckFittingCoefficients(fittingLambda) == false)
					std::cout << "    - WARNING: unable to fit the thermal conductivity..." << std::endl;
				else
					is_lambda_feasible = true;
			}

			// Fitting Mass Diffusivities
			if (is_gamma_feasible == false)
			{
				std::cout << "    - Mass diffusion coefficients..." << std::endl;
				...
				for (unsigned int j=1;j<=this->NC;j++)
				{
					...
					if (CheckFittingCoefficients(fittingBinaryDiffusivities[j-1]) == false)
					{
						std::cout << "    - WARNING: unable to fit the mass diffusion coefficients..." << std::endl;
						is_gamma_feasible = false;
						break;
					}
				}
			...
			if (is_teta_feasible == false)
			{
				std::cout << "    - Thermal diffusion ratios..." << std::endl;
				...
				if (NT>0)
				{
					...
					if (CheckFittingCoefficients(fittingTetaBinary) == false)
						std::cout << "    - WARNING: unable to fit the thermal diffusion ratios..." << std::endl;
					else
						is_teta_feasible = true;
				}
				else
				{
					is_teta_feasible = true;
			...
			if (is_eta_feasible == true && is_lambda_feasible == true && is_gamma_feasible == true && is_teta_feasible == true)
				break;
		}
		...
		if (is_eta_feasible == true && is_lambda_feasible == true && is_gamma_feasible == true && is_teta_feasible == true)
			...
			OpenSMOKE::FatalErrorMessage("Fitting of transport properties failed! Please check the input transport file.");
