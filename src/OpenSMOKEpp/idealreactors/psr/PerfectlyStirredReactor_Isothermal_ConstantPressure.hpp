...
namespace OpenSMOKE
{
	...
	ODESystem_BzzOde_PerfectlyStirredReactor* ptOde_PerfectlyStirredReactor_Isothermal_ConstantPressure;
	void ODE_Print_PerfectlyStirredReactor_Isothermal_ConstantPressure(BzzVector &Y, double t)
	{
		ptOde_PerfectlyStirredReactor_Isothermal_ConstantPressure->MyPrint(Y,t);
	}
	#endif
	
	PerfectlyStirredReactor_Isothermal_ConstantPressure::PerfectlyStirredReactor_Isothermal_ConstantPressure(	
								...
								OpenSMOKE::OscillatingPerfectlyStirredReactor& oscillating_psr,
		...
		PerfectlyStirredReactor(thermodynamicsMap, kineticsMap, ode_parameters, psr_options, on_the_fly_ropa, on_the_fly_post_processing, polimi_soot_analyzer, oscillating_psr)
	{
		type_ = PERFECTLYSTIRRED_REACTOR_ISOTHERMAL_CONSTANTP;
		...
		// Oscillating PSR
		if (oscillating_psr_.is_active() == true)
		{
			oscillating_psr_.SetInitialConditions(T0_, P_, tau0_);
			oscillating_psr_.CompleteSetup();
			oscillating_psr_.PrepareOutputFiles(psr_options_.output_path());
			if (polimi_soot_analyzer_.is_active() == true)	oscillating_psr_.PrepareOutputFilesPolimiSoot(psr_options_.output_path());
			oscillating_psr_.Summary(std::cout);
		}
	...
	int PerfectlyStirredReactor_Isothermal_ConstantPressure::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		...
		// Oscillating PSR
		if (oscillating_psr_.is_active())
		{
			oscillating_psr_.Update(t);
			tau_ = oscillating_psr_.tau();
			T_ = oscillating_psr_.T();
		}
	...
	int PerfectlyStirredReactor_Isothermal_ConstantPressure::Print(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		iteration_++;
		...
		if (psr_options_.verbose_output() == true)
		{
			if (psr_options_.verbose_ascii_file() == true)
			{
				...
				if (oscillating_psr_.is_active() == true)
				{
					if (iteration_ % oscillating_psr_.n_steps_output() == 1 ||
						oscillating_psr_.n_steps_output() == 1)
					{
						oscillating_psr_.Print(t, T_, P_, tau_, x_.GetHandle(), omega_.GetHandle());

						if (polimi_soot_analyzer_.is_active() == true)
							oscillating_psr_.Print(polimi_soot_analyzer_, t, T_, P_, tau_, rho_, x_.GetHandle(), omega_.GetHandle());
					}
				}
			}
		}
	...
	void PerfectlyStirredReactor_Isothermal_ConstantPressure::PrintFinalStatus(std::ostream& fOutput)
	{
	...
	void PerfectlyStirredReactor_Isothermal_ConstantPressure::PrintParametricFinalStatus(std::ostream& fOutput)
	{
		fOutput << std::setw(20) << std::left << tau_;
		fOutput << std::setw(20) << std::left << T0_;
		fOutput << std::setw(20) << std::left << P0_;
	...
	void PerfectlyStirredReactor_Isothermal_ConstantPressure::PrintSolution()
	{
		if (psr_options_.verbose_output() == true)
		{
			// ASCII file output
	...
	void PerfectlyStirredReactor_Isothermal_ConstantPressure::Solve(const double tf)
	{
		if (psr_options_.verbose_video() == true)
		{
			std::cout << std::endl;
		...
		else if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_BZZODE)
		{
			...
			ptOde_PerfectlyStirredReactor_Isothermal_ConstantPressure = &odepsr;
			o.StepPrint(ODE_Print_PerfectlyStirredReactor_Isothermal_ConstantPressure);
	...
	void PerfectlyStirredReactor_Isothermal_ConstantPressure::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		// Scaling factors
