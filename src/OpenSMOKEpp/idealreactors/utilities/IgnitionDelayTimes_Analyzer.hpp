	...
	void IgnitionDelayTimes_Analyzer::PrintOnFile(const boost::filesystem::path file_name)
	{
		...
		if (is_rcm_ == true)
		{
			// Preliminary filtering
			{
				...
				const unsigned int n = static_cast<unsigned int>(history_time_.size());
			...
			// Regularization
			{
				...
				FixedProfile profile_temperature(static_cast<unsigned int>(history_time_.size()), history_time_.data(), history_temperature_.data());
				...
				// Search for maxima
				{
					...
					const unsigned int n = static_cast<unsigned int>(reg_time.size());
