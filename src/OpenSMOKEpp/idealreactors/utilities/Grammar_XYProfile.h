...
namespace OpenSMOKE
{
	...
	double Conversion(const ConversionType index, const std::string variable, const std::string units)
	{
		...
		else if (index == ConversionType::CONVERSIONTYPE_Y)
		{
			...
			else if (variable == "heat-transfer-coefficient")
			{
				if (units == "W/m2/K")			conversion = 1.;
				else if (units == "W/m2/C")		conversion = 1.;
				else if (units == "kcal/m2/K")	conversion = 4186.8;
				else if (units == "kcal/m2/C")	conversion = 4186.8;
				else OpenSMOKE::FatalErrorMessage("Unknown heat-transfer-coefficient. Allowed units: W/m2/K | W/m2/K | kcal/m2/K | kcal/m2/C");
			}
		...
		std::string& x_variable, std::string& y_variable)
	{
		...
		if (dictionary.CheckOption("@FilterWidth") == true)
		{
			...
			ApplyFilter(width, 5, static_cast<unsigned int>(x.size()), x_new.data(), y_new.data(), x.data(), y.data());
			x = x_new;
			y = y_new;
		}
	}
