...
namespace OpenSMOKE
{
	...
	protected:
		...
		virtual void DefineRules()
		{
			AddKeyWord( OpenSMOKE::OpenSMOKE_DictionaryKeyWord("@Type", 
																...
																"Type of parameter to be analyzed: residence-time | temperature | pressure | temperature-pressure | residence-time-temperature | residence-time-pressure |moles | masses", 
																true) );	
	...
	void ParametricAnalysis_Options::SetupFromDictionary(OpenSMOKE::OpenSMOKE_Dictionary& dictionary)
	{
		...
		if (dictionary.CheckOption("@Type") == true)
		{
			...
			else if (value == "residence-time-temperature")
				parameter_type_ = PARAMETER_TYPE_TIME_TEMPERATURE;
			else if (value == "residence-time-pressure")
				parameter_type_ = PARAMETER_TYPE_TIME_PRESSURE;
			else
		...
		else
		{
			if (dictionary.CheckOption("@ListOfValues") == true)
			{
				...
				else if (parameter_type_ == PARAMETER_TYPE_TIME || parameter_type_ == PARAMETER_TYPE_TIME_TEMPERATURE || parameter_type_ == PARAMETER_TYPE_TIME_PRESSURE)
				...
				if (dictionary.CheckOption("@ListOfValues2") == true)
				{
					...
					Eigen::VectorXd list_of_values;
					if (parameter_type_ == PARAMETER_TYPE_TEMPERATURE_PRESSURE || parameter_type_ == PARAMETER_TYPE_TIME_PRESSURE)
						list_of_values = ReadListOfValues(values, PARAMETER_TYPE_PRESSURE);
					else if (parameter_type_ == PARAMETER_TYPE_TIME_TEMPERATURE)
						list_of_values = ReadListOfValues(values, PARAMETER_TYPE_TEMPERATURE);
			...
			else
			{
				...
				number_of_parameters_ = static_cast<int>(minimum_values_.size());
