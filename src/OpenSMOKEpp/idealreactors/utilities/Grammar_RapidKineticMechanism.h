...
namespace OpenSMOKE
{
	...
	protected:
		...
		virtual void DefineRules()
		{
			...
			AddKeyWord( OpenSMOKE::OpenSMOKE_DictionaryKeyWord("@Liquid", 
																OpenSMOKE::SINGLE_PATH, 
																"Name of the file (ASCII) containing the liquid mechanism (CHEMKIN-like format)", 
																false,
																"none",
																"@Kinetics",
																"@Surface @Solid") );
	...
	void RapidKineticMechanismWithoutTransport(	boost::filesystem::path path_output, 
							...
							const boost::filesystem::path liquid_file = "") 
	{
		...
		if (liquid_file != "")
		{
			...
			CheckForFatalError( preprocessor_liquid_kinetics.ReadFromASCIIFile(liquid_file.string()) );
			...
			if (write_ascii_short_kinetic_summary_ == true)
			{
				...
				preprocessor_liquid_kinetics.WriteShortSummaryOnASCIIFile(short_summary_kinetics.string(), *liquid_preprocessor_species_without_transport);
                        ...
                        delete liquid_preprocessor_species_without_transport;
		}
	    ...
	    // Preprocessing the liquid kinetic mechanism
            if (liquid_file != "")
            {
                    ...
                    PreProcessorSpecies_CHEMKIN_WithTransport* liquid_preprocessor_species_with_transport;
                    liquid_preprocessor_species_with_transport = new PreProcessorSpecies_CHEMKIN_WithTransport(*thermoreader, preprocessor_kinetics.names_species(), preprocessor_liquid_kinetics, fLog);
                    CheckForFatalError( liquid_preprocessor_species_with_transport->Setup() );
                    CheckForFatalError( preprocessor_liquid_kinetics.ReadKineticsFromASCIIFile( liquid_preprocessor_species_with_transport->AtomicTable(), preprocessor_kinetics ) );
                    ...
                    // Write on XML files
                    {
                            ...
                            // Thermodynamics properties
                            liquid_preprocessor_species_with_transport->WriteXMLFile(xml_string);
                    ...
                    {
                            boost::filesystem::path short_summary_kinetics   = path_output / "LiquidSummary.out";
                            preprocessor_liquid_kinetics.WriteShortSummaryOnASCIIFile(short_summary_kinetics.string(), *liquid_preprocessor_species_with_transport);
                    }

                    delete liquid_preprocessor_species_with_transport;
            }
