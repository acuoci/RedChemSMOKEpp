...
namespace OpenSMOKE
{
		...
		PlugFlowReactor(thermodynamicsMap, kineticsMap, ode_parameters, plugflow_options, on_the_fly_ropa, on_the_fly_post_processing, idts_analyzer, polimi_soot_analyzer)
	{
		...
		momentum_equation_ = momentum_equation;
		iteration_ = 0;
        ...
        environment_temperature_ = T_environment;
		is_environment_temperature_profile_ = false;
		is_global_thermal_exchange_coefficient_profile_ = false;
		...
		QR_     = 0.;
		rho_    = rho0_;
		dP_over_dcsi_ = PressureGradient();
		csi_old_ = 0.;
		P_old_ = P_;
	...
	void PlugFlowReactor_NonIsothermal::SetEnvironmentTemperatureProfile(OpenSMOKE::PlugFlowReactor_Profile& profile)
	{
		is_environment_temperature_profile_ = true;
		environment_temperature_profile_ = &profile;
	}

	void PlugFlowReactor_NonIsothermal::SetGlobalThermalExchangeCoefficientProfile(OpenSMOKE::PlugFlowReactor_Profile& profile)
	{
		is_global_thermal_exchange_coefficient_profile_ = true;
		global_thermal_exchange_coefficient_profile_ = &profile;
	}
	...
	int PlugFlowReactor_NonIsothermal::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		...
		else
		{
			...
			{
				P_ = P_old_ + dP_over_dcsi_ * (csi_ - csi_old_);
				cTot_ = P_ / (PhysicalConstants::R_J_kmol * T_);
				Product(cTot_, x_, &c_);
				rho_ = cTot_ * MW_;
				v_ = specificmassflowrate_ / rho_;
		...
		double Qexchange = 0.;
		if (global_thermal_exchange_coefficient_ != 0. && is_environment_temperature_profile_ == false)
			Qexchange = global_thermal_exchange_coefficient_ / cross_section_over_perimeter_ * (environment_temperature_ - T_);		// [W/m3]
		else if (global_thermal_exchange_coefficient_ != 0. && is_environment_temperature_profile_ == true)
			Qexchange = global_thermal_exchange_coefficient_ / cross_section_over_perimeter_ * (environment_temperature_profile_->Get(t) - T_);		// [W/m3]
		else if (is_global_thermal_exchange_coefficient_profile_ == true && is_environment_temperature_profile_ == false)
			Qexchange = global_thermal_exchange_coefficient_profile_->Get(t) / cross_section_over_perimeter_ * (environment_temperature_ - T_);		// [W/m3]
		else if (is_global_thermal_exchange_coefficient_profile_ == true && is_environment_temperature_profile_ == true)
			Qexchange = global_thermal_exchange_coefficient_profile_->Get(t) / cross_section_over_perimeter_ * (environment_temperature_profile_->Get(t) - T_);		// [W/m3]
