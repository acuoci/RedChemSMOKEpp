...
namespace OpenSMOKE
{
	...
	void PlugFlowReactor::PreparePolimiSootFiles(const boost::filesystem::path output_file_polimi_soot, const boost::filesystem::path output_file_polimi_soot_distribution)
	{
		...
		if (polimi_soot_analyzer_.soot_classes()->is_active() == true && polimi_soot_analyzer_.formation_rates_by_classes() == true)
		{
			...
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "csi[m]", counter);
			...
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "v[m/s]", counter);
	...
	void PlugFlowReactor::PrintPolimiSoot(const double t)
	{
		...
		if (polimi_soot_analyzer_.soot_classes()->is_active() == true && polimi_soot_analyzer_.formation_rates_by_classes() == true)
		{
			...
			fPolimiSootClasses_ << std::setw(width) << std::left << tau_;
			fPolimiSootClasses_ << std::setw(width) << std::left << csi_;
	...
	double PlugFlowReactor::PressureGradient()
	{
		// Sutherland's law for viscosity
		const double mu = 1.512041288e-6 * (T_ * std::sqrt(T_)) / (T_ + 120.);

		// Roughness of inner surface
		const double epsilon = 0.;

		// Hydraulic diameter
		const double diameter = 4.*cross_section_over_perimeter_;

		// Reynolds' number
		const double Re = rho_*v_*diameter/mu;

		// Fanning friction factor
		double f = 16. / Re;
		//if (fanning_friction_correlation_ == FANNING_FRICTION_BLASIUS)
		{
			if (Re >= 2100.)
				f = 0.079 / std::pow(Re, 0.25);
			if (Re >= 1.e5)
				f = 0.0014 + 0.125 / std::pow(Re, 0.32);
		}
		//else if (fanning_friction_correlation_ == FANNING_FRICTION_CHURCHILL)
		//{
		//	const double A = std::pow(-2.457 * std::log(std::pow(7. / Re, 0.90) + 0.27 * epsilon / diameter), 16.);
		//	const double B = std::pow(37530./Re, 16.);
		//	f = 2. * std::pow(std::pow(8./Re, 12.) + std::pow(A + B, 1.5), 1./12.);
		//}
		
		// Pressure gradient (Pa/m)
		const double dP_over_dcsi = -0.50*rho_*v_*v_*f*(4./diameter);
