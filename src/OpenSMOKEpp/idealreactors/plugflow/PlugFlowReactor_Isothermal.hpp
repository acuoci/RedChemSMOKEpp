...
namespace OpenSMOKE
{
	...
	ODESystem_BzzOde_PlugFlowReactor* ptOde_PlugFlowReactor_Isothermal;
	void ODE_Print_PlugFlowReactor_Isothermal(BzzVector &Y, double t)
	{
		ptOde_PlugFlowReactor_Isothermal->MyPrint(Y,t);
	}
	#endif

	PlugFlowReactor_Isothermal::PlugFlowReactor_Isothermal(	
	...
	{
		type_ = PLUGFLOW_REACTOR_ISOTHERMAL;
	...
	void PlugFlowReactor_Isothermal::SetTemperatureProfile(OpenSMOKE::PlugFlowReactor_Profile&  profile)
	{
		fixed_temperature_profile_ = true;
		temperature_profile_ = &profile;
	}

	int PlugFlowReactor_Isothermal::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		...
		if (time_independent_ == false)	dy[NC_+1] = 1.;
		else                            dy[NC_+1] = v_;
	...
	int PlugFlowReactor_Isothermal::Print(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		iteration_++;
	...
	void PlugFlowReactor_Isothermal::PrintFinalStatus(std::ostream& fOutput)
	{
	...
	void PlugFlowReactor_Isothermal::PrintParametricFinalStatus(std::ostream& fOutput)
	{
		fOutput.setf(std::ios::scientific);
		fOutput << std::setw(20) << std::left << tau_;
		fOutput << std::setw(20) << std::left << csi_;
	...
	void PlugFlowReactor_Isothermal::Solve(const double tf)
	{
		if (plugflow_options_.verbose_video() == true)
		{
			std::cout << std::endl;
		...
		if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_OPENSMOKE)
		{
			...
			Eigen::VectorXd yMax(NE_); for (unsigned int i = 0; i<NE_; i++) yMax(i) = 1.;  yMax(NC_) = 1.e16;
		...
		else if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_BZZODE)
		{
			...
			BzzVector yMax(NE_); yMax=1.; yMax[NC_+1] = 1e16;
			...
			ptOde_PlugFlowReactor_Isothermal = &odeplugflow;
			o.StepPrint(ODE_Print_PlugFlowReactor_Isothermal);
	...
	void PlugFlowReactor_Isothermal::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		if (iteration_ == 1)
		{
