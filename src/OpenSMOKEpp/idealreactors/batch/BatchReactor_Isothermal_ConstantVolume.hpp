...
namespace OpenSMOKE
{
	...
	ODESystem_BzzOde_BatchReactor* ptOde_BatchReactor_Isothermal_ConstantVolume;
	void ODE_Print_BatchReactor_Isothermal_ConstantVolume(BzzVector &Y, double t)
	{
		ptOde_BatchReactor_Isothermal_ConstantVolume->MyPrint(Y,t);
	}
	#endif

	BatchReactor_Isothermal_ConstantVolume::BatchReactor_Isothermal_ConstantVolume(	
	...
	{
		type_ = BATCH_REACTOR_ISOTHERMAL_CONSTANTV;
	...
	int BatchReactor_Isothermal_ConstantVolume::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
	...
	int BatchReactor_Isothermal_ConstantVolume::Print(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		iteration_++;
	...
	void BatchReactor_Isothermal_ConstantVolume::Solve(const double t0, const double tf)
	{
		if (batch_options_.verbose_video() == true)
		{
			std::cout << std::endl;
		...
		if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_OPENSMOKE)
		{
			...
			if (ode_parameters_.sparse_solver() == "none")
			{
				...
				typedef OdeSMOKE::MethodGear<denseOde> denseMethodGear;
				OdeSMOKE::MultiValueSolver<denseMethodGear> ode_solver;
			...
			else
			{
				...
				typedef OdeSMOKE::MethodGear<sparseOde> sparseMethodGear;
				OdeSMOKE::MultiValueSolver<sparseMethodGear> ode_solver;
				ode_solver.SetReactor(this);
		...
		else if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_BZZODE)
		{
			...
			ptOde_BatchReactor_Isothermal_ConstantVolume = &odebatch;
			o.StepPrint(ODE_Print_BatchReactor_Isothermal_ConstantVolume);
	...
	void BatchReactor_Isothermal_ConstantVolume::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		if (iteration_ == 1)
		{
	...
	void BatchReactor_Isothermal_ConstantVolume::ChemicalExplosiveModeAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		OpenSMOKE::ErrorMessage("BatchReactor_Isothermal_ConstantVolume", "ChemicalExplosiveModeAnalysis is not yet available for Adiabtic User Defined Batch Reactor");
	}

	void BatchReactor_Isothermal_ConstantVolume::SparseAnalyticalJacobian(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, Eigen::SparseMatrix<double> &J)
	{
		// Recover mass fractions
		for (unsigned int i = 1; i <= NC_; ++i)
			omega_[i] = y[i];
		...
		// Jacobian matrix
		kineticsMap_.jacobian_sparsity_pattern_map()->Jacobian(omega_.GetHandle(), T_, P_, J);
		for (int k = 0; k < J.outerSize(); ++k)
		{
			for (Eigen::SparseMatrix<double>::InnerIterator it(J, k); it; ++it)
			{
				it.valueRef() *= thermodynamicsMap_.MW(it.row()) / rho_;
			}
		}
	}

	void BatchReactor_Isothermal_ConstantVolume::DenseAnalyticalJacobian(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, Eigen::MatrixXd &J)
	{
		OpenSMOKE::ErrorMessage("BatchReactor_Isothermal_ConstantVolume", "SparseAnalyticalJacobian is not yet available for Isothermal Constant Volume Batch Reactor");
	}
}
