...
namespace OpenSMOKE
{
	class Grammar_BatchReactor_Options : public OpenSMOKE::OpenSMOKE_DictionaryGrammar
	{
	protected:
		...
		virtual void DefineRules()
		{
			...
			AddKeyWord(OpenSMOKE::OpenSMOKE_DictionaryKeyWord("@AsciiPrecision",
																OpenSMOKE::SINGLE_INT,
																"Number of significant digits to be printed out",
																false));
		}
	};

	BatchReactor_Options::BatchReactor_Options()
	{
	...
	void BatchReactor_Options::SetupFromDictionary(OpenSMOKE::OpenSMOKE_Dictionary& dictionary)
	{
		Grammar_BatchReactor_Options grammar;
		...
		if (dictionary.CheckOption("@AsciiPrecision") == true)
			dictionary.ReadInt("@AsciiPrecision", ascii_precision_);
		...
		if (ascii_precision_ > 12 || ascii_precision_ < 6)
		{
			OpenSMOKE::FatalErrorMessage("Value of @AsciiPrecision must be between 6 and 12");
		}
	}
