...
namespace OpenSMOKE
{
	BatchReactor::BatchReactor(
		...
		on_the_fly_cema_(on_the_fly_cema),
	...
	void BatchReactor::EnableSensitivityAnalysis(OpenSMOKE::SensitivityMap& sensitivityMap, OpenSMOKE::SensitivityAnalysis_Options& sensitivity_options)
	{
	...
	void BatchReactor::PrepareROPAFile(const boost::filesystem::path output_file_ropa)
	{
		// Open ROPA file
		fROPA_.open(output_file_ropa.c_str(), std::ios::out);

		// Write head in file
		on_the_fly_ropa_.WriteHead(fROPA_, "Batch Reactor");
	...
	void BatchReactor::PreparePolimiSootFiles(const boost::filesystem::path output_file_polimi_soot, const boost::filesystem::path output_file_polimi_soot_distribution)
	{
		// Polimi Soot Analysis
		{
			...
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "t[s]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "T0[K]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "P0[Pa]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "V0[m3]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "T[K]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "P[Pa]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "V[m3]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "rho[kg/m3]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "MW[kg/kmol]", counter);
		...
		// Polimi Class Soot Analysis
		if (polimi_soot_analyzer_.soot_classes()->is_active() == true && polimi_soot_analyzer_.formation_rates_by_classes() == true)
		{
			boost::filesystem::path output_file_polimi_soot_class = output_file_polimi_soot;
			output_file_polimi_soot_class.replace_extension(".formationclasses.out");

			fPolimiSootClasses_.open(output_file_polimi_soot_class.c_str(), std::ios::out);
			fPolimiSootClasses_.setf(std::ios::scientific);
			fPolimiSootClasses_.setf(std::ios::left);
			...
			polimi_soot_analyzer_.WriteLabelFormationSootClassesDataFile(fPolimiSootClasses_, counter, width);
			fPolimiSootClasses_ << std::endl;
		}
	}

	void BatchReactor::PrepareXMLFile(const boost::filesystem::path output_file_xml)
	{
	...
	void BatchReactor::PrepareSensitivityXMLFiles(OpenSMOKE::SensitivityAnalysis_Options& sensitivity_options)
	{
		...
		const boost::filesystem::path parent_file = batch_options_.output_path() / "Sensitivities.xml";
		...
		for (unsigned int j=0;j<indices_of_sensitivity_species_.size();j++)
		{
			...
			const boost::filesystem::path child_file = batch_options_.output_path() / name;
			fSensitivityChildXML_[j].open(child_file.c_str(), std::ios::out);
		}
		{
			const boost::filesystem::path child_file = batch_options_.output_path() / "Sensitivities.temperature.xml";
	...
	void BatchReactor::PrepareASCIIFile(std::ofstream& fOutput, const boost::filesystem::path output_file_ascii)
	{
		indices_of_output_species_.resize(batch_options_.output_species().size());
		for (unsigned int i = 0; i<batch_options_.output_species().size(); i++)
			indices_of_output_species_[i] = thermodynamicsMap_.IndexOfSpecies(batch_options_.output_species()[i]);

		unsigned int safety_width = batch_options_.ascii_precision() + 8;

		if (indices_of_output_species_.size() != 0)
		{
			widths_of_output_species_.resize(batch_options_.output_species().size());
			for (unsigned int i = 0; i<batch_options_.output_species().size(); i++)			
			{
				widths_of_output_species_[i] = CalculateSpeciesFieldWidth(batch_options_.output_species()[i], NC_);
				widths_of_output_species_[i] = std::max(widths_of_output_species_[i], safety_width);
			}
		}
		else
		{
		...
		fOutput << std::setprecision(batch_options_.ascii_precision());
	...
	void BatchReactor::PrepareParametricASCIIFile(std::ofstream& fOutput, const boost::filesystem::path output_file_ascii)
	{
		indices_of_output_species_.resize(batch_options_.output_species().size());
		for (unsigned int i = 0; i<batch_options_.output_species().size(); i++)
			indices_of_output_species_[i] = thermodynamicsMap_.IndexOfSpecies(batch_options_.output_species()[i]);
	...
	void BatchReactor::PrepareParametricIDTASCIIFile(std::ofstream& fOutput, const boost::filesystem::path output_file_ascii)
	{
	...
	void BatchReactor::PrepareASCIIFile(const boost::filesystem::path output_file_ascii)
	{
	...
	void BatchReactor::PrintParametricIDT(std::ostream& fOutput, const double t)
	{
	...
	void BatchReactor::SolveOpenSourceSolvers(const double t0, const double tf)
	{
	...
	void BatchReactor::OpenAllFiles()
	{
		if (batch_options_.verbose_output() == true)
		{
			if ( !boost::filesystem::exists(batch_options_.output_path()) )
				OpenSMOKE::CreateDirectory(batch_options_.output_path());

			if (batch_options_.verbose_ascii_file() == true)
				PrepareASCIIFile(batch_options_.output_path() / "Output.out");
			
			if (batch_options_.verbose_xml_file() == true)
				PrepareXMLFile(batch_options_.output_path() / "Output.xml");

			if (on_the_fly_ropa_.is_active() == true)
				PrepareROPAFile(batch_options_.output_path() / "ROPA.out");

			if (polimi_soot_analyzer_.is_active() == true)
				PreparePolimiSootFiles(	batch_options_.output_path() / "PolimiSoot.out",
										batch_options_.output_path() / "PolimiSootDistribution.out");

			// Prepare CEMA output files
			if (on_the_fly_cema_.is_active() == true)
				on_the_fly_cema_.PrepareOutputFiles();
	...
	void BatchReactor::PrintPolimiSoot(const double t)
	{
		// Integral data
		{
			const unsigned int width = 20;
		...
		// Formation rates soot classes
		if (polimi_soot_analyzer_.soot_classes()->is_active() == true && polimi_soot_analyzer_.formation_rates_by_classes() == true)
		{
			const unsigned int width = 30;

			fPolimiSootClasses_ << std::setw(width) << std::left << t;
			fPolimiSootClasses_ << std::setw(width) << std::left << T0_;
			fPolimiSootClasses_ << std::setw(width) << std::left << P0_;
			fPolimiSootClasses_ << std::setw(width) << std::left << V0_;
			fPolimiSootClasses_ << std::setw(width) << std::left << T_;
			fPolimiSootClasses_ << std::setw(width) << std::left << P_;
			fPolimiSootClasses_ << std::setw(width) << std::left << V_;
			fPolimiSootClasses_ << std::setw(width) << std::left << rho_;
			fPolimiSootClasses_ << std::setw(width) << std::left << MW_;

			polimi_soot_analyzer_.WriteFormationSootClassesDataFile(fPolimiSootClasses_, width);

			fPolimiSootClasses_ << std::endl;
		}
	}

	void BatchReactor::CloseAllFiles()
	{
		if (batch_options_.verbose_output() == true)
		{
			if (batch_options_.verbose_ascii_file() == true)
			...
			if (on_the_fly_cema_.is_active() == true)
				on_the_fly_cema_.CloseOutputFiles();
	...
	void BatchReactor::FinalStatus(const double t0, const double tf)
	{
		...
		if (batch_options_.verbose_video() == true)
		{
			...
			// Atomic analysis
			{
				...
				if (sum_initial[j] > 0.)
				{
					std::string label = "Element(kmol) " + thermodynamicsMap_.elements()[j];
	...
	void BatchReactor::FinalSummary(const boost::filesystem::path summary_file, const double t0, const double tf)
	{
		OpenSMOKE::OpenSMOKEVectorDouble dummy(NE_);
		Equations(tf, yf_, dummy);
	...
	void BatchReactor::PolimiSootAnalysis(const double t)
	{
		...
		// Analysis of formation rates in classes
		polimi_soot_analyzer_.Analysis(kineticsMap_, T_, P_, rho_, Yeigen, Xeigen);
