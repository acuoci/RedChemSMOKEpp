...
namespace OpenSMOKE
{
	...
	void ShockTubeReactor::PreparePolimiSootFiles(const boost::filesystem::path output_file_polimi_soot, const boost::filesystem::path output_file_polimi_soot_distribution)
	{
		...
		if (polimi_soot_analyzer_.soot_classes()->is_active() == true && polimi_soot_analyzer_.formation_rates_by_classes() == true)
		{
			...
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "z[m]", counter);
			OpenSMOKE::PrintTagOnASCIILabel(width, fPolimiSootClasses_, "tLab[s]", counter);
	...
	void ShockTubeReactor::PrintPolimiSoot(const double t)
	{
		...
		if (polimi_soot_analyzer_.soot_classes()->is_active() == true && polimi_soot_analyzer_.formation_rates_by_classes() == true)
		{
			...
			fPolimiSootClasses_ << std::setw(width) << std::left << v_;
			fPolimiSootClasses_ << std::setw(width) << std::left << z_;
