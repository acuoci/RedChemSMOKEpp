...
namespace OpenSMOKE
{
	...
	*/
	...
	public:
		...
		*@param nPoints number op points in the map (for a scalar map the number of oints is 1)
		*/
		KineticsMap_CHEMKIN(ThermodynamicsMap_CHEMKIN& thermo, boost::property_tree::ptree& ptree, const unsigned int nPoints = 1);
		...
		*@param nPoints number op points in the map (for a scalar map the number of oints is 1)
		*/
		KineticsMap_CHEMKIN(ThermodynamicsMap_CHEMKIN& thermo, boost::property_tree::ptree& ptree, bool verbose, const unsigned int nPoints = 1);
		...
		*/
		...
		/**
		*@brief Returns the indices of the reversible reactions (the returned index is 1-based)
		*/
		...
		*/
		void KineticConstants(const std::vector<double>& cc, const std::vector<double>& cc_falloff, const std::vector<double>& cc_reverse);

		/**
		*@brief Return the frequency factor of a single reaction [kmol, m, s]
		*@param j index of reaction (starting from zero)
		*/
		double A(const unsigned int j) const { return sign_lnA__[j] * std::exp(lnA__[j]); }
		...
		*@param j index of reaction (starting from zero)
		*/
		double A_falloff_inf(const unsigned int j) const { return  std::exp(lnA_falloff_inf__[j]); }

		/**
		*@brief Return the temperature exponent of the high pressure limit of a single reaction
		*@param j index of reaction (starting from zero)
		*/
		double Beta_falloff_inf(const unsigned int j) const { return Beta_falloff_inf__[j]; }

		/**
		*@brief Return the activation temperature of the high pressure limit of a single reaction [K]
		*@param j index of reaction (starting from zero)
		*/
		double E_over_R_falloff_inf(const unsigned int j) const { return E_over_R_falloff_inf__[j]; }
		...
		*/
		...
		/**
		*@brief Returns the dimensionless reaction enthalpy
		*@param j reaction index (from 0)
		*/
		double reaction_h_over_RT(const unsigned int j) const { return reaction_h_over_RT__[j]; }

		/**
		*@brief Returns the net change of moles in the reaction
		*@param j reaction index (from 0)
		*/
		double changeOfMoles(const unsigned int j) const { return changeOfMoles__[j]; }
