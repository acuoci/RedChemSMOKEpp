...
#ifndef DaeMethodGear_H
#define DaeMethodGear_H
...
namespace DaeSMOKE
{
	//!  A class implementing the Gear methods to solve stiff DAE systems
	/*!
	The purpose of this class is to solve a stiff DAE system using the Gear methods.
	The class is based on the DAESystemKernel policy, which implements the functions to solve the linear systems
	requested by the Gear methods.
	*/

	template <typename DAESystemKernel>
	class MethodGear : public DAESystemKernel
	{
	public:
		...
		*/
		void DaeMethodSummary(std::ostream& out);

		#if LOG_MODE == 1
			std::ofstream fLog;
			std::deque<double> hHistory_;
		#endif
	...
	protected:
		...
		*@param one_over_epsilon_system error vector: one_over_epsilon = (tolAbs + tolRel*abs(y))^(-1)
		*/
		unsigned int FindCorrection(const double t, const Eigen::VectorXd& one_over_epsilon, const Eigen::VectorXd& one_over_epsilon_system);
	...
	protected:
		...
		Eigen::VectorXd* r_;			//!< vector r characteristic of the method (there is a vector for each order which was coded, up to MAX_ORDER_DAE)
		...
		Eigen::VectorXd vc_;				//!< auxiliary vector
		...
		DaeHStatus daeHStatus_;				//!< status of the step size (decreased, constant, or decreased)
		DaeOrderStatus daeOrderStatus_;		//!< status of the order (decreased, constant, or decreased)
		...
		// Status
		DaeConvergence convergenceStatus_;						//!< status of the convergence procedure (success or failure)
		DaeStatus status_;										//!< status of the DAE system
		...
		// Default values
		static const unsigned int MIN_ORDER_DAE = 1;					//!< minimum order of the method
		static const unsigned int MAX_ORDER_DAE = 5;					//!< maximum order which was coded in the current implementation 
