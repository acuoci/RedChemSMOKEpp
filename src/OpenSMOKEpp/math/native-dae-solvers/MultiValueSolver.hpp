...
namespace DaeSMOKE
{
	...
	#if LOG_MODE == 1
	const unsigned int hHistoryMaxSize = 100;
	#endif
	...
	void MultiValueSolver<Method>::SetDefaultValues()
	{
		...
		this->maximum_order_ = this->MAX_ORDER_DAE;
		...
		this->status_ = DAE_STATUS_TO_BE_INITIALIZED;
	}
	...
	void MultiValueSolver<Method>::Reset()
	{
		...
		this->daeHStatus_     = H_STATUS_CONST;
		this->daeOrderStatus_ = ORDER_STATUS_CONST;
	}
	...
	void MultiValueSolver<Method>::SetMaximumOrder(const unsigned int maximum_order)
	{
		...
		if (maximum_order > this->MAX_ORDER_DAE)
			this->maximum_order_ = this->MAX_ORDER_DAE;
		if (maximum_order < 1)
			this->maximum_order_ = this->MAX_ORDER_DAE;
	}
	...
	template <typename Method>
	void MultiValueSolver<Method>::RescaleTolerances()
	{
		abs_tolerances_.resize(0);
		rel_tolerances_.resize(0);

		SetRelativeTolerances(DEFAULT_TOL_REL);
		SetAbsoluteTolerances(DEFAULT_TOL_ABS);
	}
	...
	template <typename Method>
	unsigned int MultiValueSolver<Method>::CheckIllegalConstraints(const Eigen::VectorXd& y)
	{
		unsigned int number_of_illegal_constraints_ = 0;

		if (this->min_constraints_ == true)
		{
			if (abs_tolerances_scalar_ == true)
			{
				for (unsigned int i = 0; i < this->ne_; i++)
				if ((y(i) + abs_tolerance_) < this->min_values_(i))
				{
					std::cout << std::scientific << "Illegal minimum: " << " eq=" << i << " v=" << y(i) << " min=" << this->min_values_(i) << " absTol=" << abs_tolerance_ << std::endl;
					number_of_illegal_constraints_++;
				}
			}
			else if (abs_tolerances_scalar_ == false)
			{
				for (unsigned int i = 0; i < this->ne_; i++)
				if ((y(i) + abs_tolerances_(i)) < this->min_values_(i))
				{
					std::cout << std::scientific << "Illegal minimum: " << " eq=" << i << " v=" << y(i) << " min=" << this->min_values_(i) << " absTol=" << abs_tolerances_(i) << std::endl;
					number_of_illegal_constraints_++;
				}
			}
		}
		...
		if (this->max_constraints_ == true)
		{
			...
			{
				for (unsigned int i = 0; i < this->ne_; i++)
					if ((y(i) - abs_tolerance_) > this->max_values_(i))
					{
						std::cout << std::scientific << "Illegal maximum: " << " eq=" << i << " v=" << y(i) << " max=" << this->max_values_(i) << " absTol=" << abs_tolerance_ << std::endl;
						number_of_illegal_constraints_++;
					}
			}
			else if (abs_tolerances_scalar_ == false)
			{
				for (unsigned int i = 0; i < this->ne_; i++)
					if ((y(i) - abs_tolerances_(i)) > this->max_values_(i))
					{
						std::cout << std::scientific << "Illegal maximum: " << " eq=" << i << " v=" << y(i) << " max=" << this->max_values_(i) << " absTol=" << abs_tolerances_(i) << std::endl;
						number_of_illegal_constraints_++;
					}
			}
		}
	...
	template <typename Method>
	void MultiValueSolver<Method>::SetInitialConditions(const double t0, const Eigen::VectorXd& y0, const std::vector<OpenSMOKE::EquationType>& equation_type)
	{
		...
		// Total number of equations (TODO)
		...
		if (memory_allocation_is_needed == true)
		{
			...
			one_over_epsilon_system_.resize(this->ne_);
	...
	template <typename Method>
	DaeStatus MultiValueSolver<Method>::Solve(const double tOut)
	{
		if (this->status_ < 0)
		{
		...
		if (calculation_with_strict_tmax_ == true)
		{
			...
			{
				this->status_ = DAE_STATUS_ILLEGAL_MAX_INDEPENDENT_VARIABLE;
				ParseDaeStatus();
				return this->status_;
			}
		}
		...
		if (this->status_ == DAE_STATUS_TO_BE_INITIALIZED)
		{
			// Check the constraints on the initial conditions
			if (this->CheckIllegalConstraints(y0_) != 0)
			{
				this->status_ = DAE_STATUS_ILLEGAL_CONSTRAINTS;
				ParseDaeStatus();
				return this->status_;
			}
			...
			for (unsigned int i = 0; i<this->ne_; i++)
				if (this->equation_type_[i] == OpenSMOKE::EQUATION_TYPE_ALGEBRAIC)
					this->z_[1](i) = 0.;
			...
			// Update the status
			this->status_ = DAE_STATUS_CONTINUATION;
		}
		// Continuation (i.e. the integration is restarted without chenging the initial conditions)
		else
		{
			// Check the constraints on the initial conditions
				 ...
				 (direction_plus_ == false && tOut_ > t_)  )
			{
					...
					(direction_plus_ == false && tOut_ > t_ - hPreviousStep_))
				{
					...
					if (direction_plus_ == true)
						std::cout << "The continuation procedure is allowed only for tOut >= " << t_ - hPreviousStep_ << std::endl;
					else
						std::cout << "The continuation procedure is allowed only for tOut <= " << t_ - hPreviousStep_ << std::endl;
				
					this->status_ = DAE_STATUS_ILLEGAL_CONTINUATION_REQUEST;
					ParseDaeStatus();
					return this->status_;
				}
			...
			if (calculation_with_strict_tmax_ == true)
			{
					...
					(direction_plus_ == false && t_ + this->hNextStep_ < tStrictMax_))
				{
					...
					this->daeHStatus_ = H_STATUS_DECREASED;
				}
			}
		}

		#if LOG_MODE == 1
				this->fLog.open("dae.log", std::ios::out);
				this->fLog.setf(std::ios::scientific);
		#endif

		// Multivalue steps
		for (unsigned int k = 1; k <= max_number_steps_; k++)
		{
			if (stopDaeIntegration == true)
			{
				this->status_ = DAE_STATUS_EXCEPTION_HANDLING_STOP;
				ParseDaeStatus();
				return this->status_;
			}
			...
			CalculatesOneOverEpsilonSystem(this->z_[0]);
			...
			if (tolSafe > 1.)
			{
				RescaleTolerances();
				CalculatesOneOverEpsilon(this->z_[0]);
				tolSafe = OpenSMOKE::OPENSMOKE_MACH_EPS_DOUBLE*OpenSMOKE::ErrorControl(this->z_[0], one_over_epsilon_);
				if (tolSafe > 1.)
				{
					this->status_ = DAE_STATUS_TOO_STRICT_TOLERANCES;
					ParseDaeStatus();
					return this->status_;
				}
			}
			...
			#if LOG_MODE == 1
			this->fLog << "---------------------------------------" << std::endl;
			this->fLog << "Step: " << k  << std::endl;
			this->fLog << "Time: " << t_ << std::endl;
			this->fLog << "---------------------------------------" << std::endl;
			#endif

			// Advances the solution of the DAE's by one integration step
			DriverStep();

			// If the norm of the first order derivatives is sufficiently small, the integration is completed with success
			if (this->status_ == DAE_STATUS_STOP_INTEGRATION_BEFORE_RECALCULATING_JACOBIAN)
			{
				this->y_ = this->z_[0];
				tOut_ = t_;
				return this->status_;
			...
			// If the norm of the first order derivatives is sufficiently small, the integration is completed with success
			if (this->status_ == DAE_STATUS_STOP_INTEGRATION_FOR_SMALL_YPRIME_NORM1)
			{
				this->y_ = this->z_[0];
				tOut_ = t_;
				return this->status_;
		...
		this->status_ = DAE_STATUS_MAX_NUMBER_OF_STEPS_REACHED;
		ParseDaeStatus();
		return this->status_;
	}
	...
	void MultiValueSolver<Method>::CalculatesOneOverEpsilon(const Eigen::VectorXd& w)
	{
		...
		{
			for (unsigned int i = 0; i<this->ne_; i++)
				one_over_epsilon_(i) = abs_tolerance_ + rel_tolerance_*w(i);
		}
		...
		for (unsigned int i = 0; i < this->ne_; i++)
		{
			const double coefficient = one_over_epsilon_(i);

			if (coefficient == 0) 
				FatalErrorMessage("CalculatesOneOverEpsilon: Improve float consistency...");

			if (this->equation_type_[i] == OpenSMOKE::EQUATION_TYPE_DIFFERENTIAL)
				one_over_epsilon_(i) = 1. / coefficient;
			else
				one_over_epsilon_(i) = 0.;
		}
	}

	template <typename Method>
	void MultiValueSolver<Method>::CalculatesOneOverEpsilonSystem(const Eigen::VectorXd& w)
	{
		
		if (abs_tolerances_scalar_ == true && rel_tolerances_scalar_ == true)
		{
			const double abs_tol_ = std::min(abs_tolerance_, DEFAULT_TOL_ABS);
			const double rel_tol_ = std::min(rel_tolerance_, DEFAULT_TOL_REL);

			for (unsigned int i = 0; i<this->ne_; i++)
				one_over_epsilon_system_(i) = abs_tol_ + rel_tol_*w(i);
		...
		else if (abs_tolerances_scalar_ == false && rel_tolerances_scalar_ == true)
		{
			...
			for (unsigned int i = 0; i < this->ne_; i++)
			{
				const double abs_tol_ = std::min(abs_tolerances_(i), DEFAULT_TOL_ABS);
				one_over_epsilon_system_(i) = abs_tol_ + rel_tol_*w(i);
			}
		}
		else if (abs_tolerances_scalar_ == true && rel_tolerances_scalar_ == false)
		{
			...
			for (unsigned int i = 0; i < this->ne_; i++)
			{
				const double rel_tol_ = std::min(rel_tolerances_(i), DEFAULT_TOL_REL);
				one_over_epsilon_system_(i) = abs_tol_ + rel_tol_*w(i);
			}
		}
		else if (abs_tolerances_scalar_ == false && rel_tolerances_scalar_ == false)
		...
		for (unsigned int i = 0; i < this->ne_; i++)
		{
			const double coefficient = one_over_epsilon_system_(i);

			if (coefficient == 0)
				FatalErrorMessage("CalculatesOneOverEpsilonSystem: Improve float consistency");

			if (this->equation_type_[i] == OpenSMOKE::EQUATION_TYPE_DIFFERENTIAL)
				one_over_epsilon_system_(i) = 1. / coefficient;
			else
				one_over_epsilon_system_(i) = 0.01 / coefficient;
		}
	}
	...
	template <typename Method>
	void MultiValueSolver<Method>::ParseDaeStatus()
	{
		switch (this->status_)
		{
			case DAE_STATUS_MAX_NUMBER_OF_STEPS_REACHED:
				WarningMessage("The ODE system integration reached the maximum number of steps.");
				break;

			case DAE_STATUS_TOO_STRICT_TOLERANCES:
				WarningMessage("The required tolerances are too strict. Try to relax them!");
				break;

			case DAE_STATUS_ILLEGAL_CONTINUATION_REQUEST:
				WarningMessage("The continuation request cannot be performed because of illegal value.");
				break;

			case DAE_STATUS_MAX_NUMBER_ERRORTEST_FAILURES:
				WarningMessage("The maximum number of error test failures was reached.");
				break;

			case DAE_STATUS_MAX_NUMBER_CONVERGENCETEST_FAILURES:
				WarningMessage("The maximum number of convergence test failures was reached.");
				break;

			case DAE_STATUS_TOO_SMALL_STEP_SIZE:
				{
					std::cout << "Current time step size:    " << this->h_ << std::endl;
					std::cout << "Minimum allowed step size: " << this->min_step_size_ << std::endl;

					//if (calculation_with_strict_tmax_ == false)
					WarningMessage("The current step is equal to (or smaller than) the minimum allowed step.");
				}
				break;

			case DAE_STATUS_ILLEGAL_MAX_INDEPENDENT_VARIABLE:
				WarningMessage("The requested maximum value of the indepepndent variable is illegal.");
				break;

			case DAE_STATUS_ILLEGAL_CONSTRAINTS:
				WarningMessage("The provided constraint on maximum and/or minimum values are not satisfied.");
				break;

			case DAE_STATUS_STOP_INTEGRATION_BEFORE_RECALCULATING_JACOBIAN:
				WarningMessage("Integration stopped before recalculating the Jacobian matrix.");
				break;

			case DAE_STATUS_EXCEPTION_HANDLING_STOP:
	...
	void MultiValueSolver<Method>::DriverStep()
	{
		...
		if (this->daeOrderStatus_ != ORDER_STATUS_CONST || this->daeHStatus_ != H_STATUS_CONST)
			this->iterOrder_ = 0;
		...
		#if LOG_MODE == 1
		this->fLog << " * order:    " << this->p_ << std::endl;
		this->fLog << " * order it: " << this->iterOrder_ << std::endl;
		#endif
		...
		#if LOG_MODE == 1
		this->fLog << " * numberOfSteps: " << this->numberOfSteps_ << std::endl;
		this->fLog << " * hScaleMax: " << hScaleMax_ << std::endl;
		#endif

		while (1)
		{
			#if LOG_MODE == 1
			this->fLog << " * New step:    " << this->hNextStep_ << std::endl;
			this->fLog << " * Nord step:   " << hNordsieck_ << std::endl;
			this->fLog << " * Step const?: " << this->daeHStatus_ << std::endl;
			#endif

			// If the step was modified, we have to update the z vector
			// See Buzzi-Ferraris, page 675
			if (this->daeHStatus_ != H_STATUS_CONST)
			{
			...
			#if LOG_MODE == 1
			this->fLog << " * h: " << this->h_ << std::endl;
			this->fLog << " * min(h): " << OpenSMOKE::OPENSMOKE_MACH_EPS_FLOAT*this->min_step_size_ << std::endl;
			#endif
			...
			{
				this->status_ = DAE_STATUS_TOO_SMALL_STEP_SIZE;
				return;
			}
			...
			#if LOG_MODE == 1
			this->fLog << " * t: " << t_ << " " << tInMeshPoint_ << " " << this->h_ << std::endl;
			#endif
			...
			if (stopIntegrationForSmallYPrimeNorm1_ == true)
			{
				...
				#if LOG_MODE == 1
				this->fLog << " * norm1: " << norm1 << std::endl;
				#endif

				// If the norm is sufficiently small, the integration is successfully completed
				if (norm1 < YPrimeNorm1_)
				{
					this->status_ = DAE_STATUS_STOP_INTEGRATION_FOR_SMALL_YPRIME_NORM1;
					return;
				}
			}

			#if LOG_MODE == 1
			this->fLog << " * multivalue: ";
			#endif
			...
			#if LOG_MODE == 1
				if (this->hHistory_.size() >= hHistoryMaxSize)
				{
					const double hAverage = std::accumulate(this->hHistory_.begin(), this->hHistory_.end(), 0.) / static_cast<double>(hHistoryMaxSize);

					if (hAverage / this->t_ < 1e-4)
					{
						std::cout << " ==> History" << std::endl;
						std::cout << " ==> hAverage: "	<< std::scientific << hAverage 
								  << " t: "				<< std::scientific << this->t_ 
								  << " ratio: "			<< std::scientific << hAverage / this->t_ << std::endl;

						
						this->hHistory_.resize(0);
						this->convergenceStatus_ = CONVERGENCE_STATUS_FAILURE;
					}
				}
			#endif

			#if LOG_MODE == 1
			this->fLog << this->status_ << std::endl;
			this->fLog << " * convergence: " << this->convergenceStatus_ << std::endl;

				if (this->status_ == DAE_STATUS_STOP_INTEGRATION_BEFORE_RECALCULATING_JACOBIAN)
					this->fLog << " * Stop before recal Jac" << std::endl;
			#endif
			...
			if (this->convergenceStatus_ == CONVERGENCE_STATUS_FAILURE)
			{
				...
				if (std::fabs(this->h_ - this->min_step_size_) <= OpenSMOKE::OPENSMOKE_MACH_EPS_DOUBLE*std::fabs(this->min_step_size_))
				{
					#if LOG_MODE == 1
					this->fLog << " * Failure" << std::endl;
					this->fLog << " * TOO_SMALL_STEP_SIZE" << std::endl;
					#endif
			...
			else
			{
				...
				#if LOG_MODE == 1
				this->fLog << " * convergenceSuccess: " << numberOfConvergenceSuccess_ << std::endl;
				#endif
				...
				#if LOG_MODE == 1
				this->fLog << " * hScaleCurrent: " << hScaleCurrent << std::endl;
				#endif
				...
				if (hScaleCurrent > 1.)
				{
					...
					{
						this->status_ = DAE_STATUS_MAX_NUMBER_ERRORTEST_FAILURES;
						return;
					}

					// If the current time step is too small, it cannot be further decreased
					...
					// A safety coefficient equal to 0.8 is included
					{
						...
						if (this->p_ > this->MIN_ORDER_DAE)
						{
							...
							if (hScaleMinus > hScaleCurrent)
							{
								...
								this->p_ -= 1;
								this->daeOrderStatus_ = ORDER_STATUS_DECREASED;
							}
						}
					}
					...
					#if LOG_MODE == 1
					this->fLog << " * hScale: " << this->hScale_ << std::endl;
					#endif
					...
					if (100.*cpuTimeEquationSystem_ < cpuTimeToFactorize_ && cpuTimeToFactorize_ > 1.)
						...
						if (100.*cpuTimeEquationSystem_ < cpuTimeToFactorize_ && cpuTimeToFactorize_ > 1.)
							criterion2 = true;
						this->fLog << " * cpuTimeEquationSystem " << cpuTimeEquationSystem_ << std::endl;
						this->fLog << " * cpuTimeToFactorize    " << cpuTimeToFactorize_ << std::endl;
						this->fLog << " * criterion1: " << criterion1 << std::endl;
						this->fLog << " * criterion2: " << criterion2 << std::endl;
						this->fLog << " * hNextStep   " << this->hNextStep_ << std::endl;
					#endif
				...
				else
				{
...
#if LOG_MODE == 1
					this->fLog << " * step(old/new): " << this->h_ << " " << this->hNextStep_ << std::endl;
#endif
					...
					if (calculation_with_strict_tmax_ == true)
					{
						...
						if (this->h_ != this->hNextStep_)
						{
							if (std::fabs(this->hNextStep_) > std::fabs(this->h_))
								this->daeHStatus_ = H_STATUS_INCREASED;
							else
								this->daeHStatus_ = H_STATUS_DECREASED;
						}
					}
					...
					else
					{
						...
						#if LOG_MODE == 1
						this->fLog << " * new order: " << this->p_ << std::endl;
						#endif
					}
					...
					#if LOG_MODE == 1
						// Track history of step sizes
						{
							if (this->hHistory_.size() >= hHistoryMaxSize)
								this->hHistory_.pop_front();
							this->hHistory_.push_back(this->h_);
						}
					#endif
	...
	void MultiValueSolver<Method>::NewOrderNewH()
	{
		...
		// Choosing the best new time step (see Buzzi-Ferraris, page 680)

		// Order p-1
		if (this->p_ > this->MIN_ORDER_DAE && this->daeOrderStatus_ != ORDER_STATUS_INCREASED)
		{
		...
		// Order p+1
		if (this->p_ < this->maximum_order_ &&	this->daeOrderStatus_ != ORDER_STATUS_DECREASED)
		{
			// Estimate the error e corresponding to order p+1
		...
		#if LOG_MODE == 1
		this->fLog << "   >----> hScale(p-1): " << hScaleMinus << std::endl;
		this->fLog << "   >----> hScale(p):   " << hScaleCurrent << std::endl;
		this->fLog << "   >----> hScale(p+1): " << hScalePlus << std::endl;
		#endif
		...
		else
		{
			...
			this->orderInNextStep_ = this->p_ + 1;
			this->daeOrderStatus_ = ORDER_STATUS_INCREASED;
		}

		#if LOG_MODE == 1
			this->fLog << "   >----> hScale: " << this->hScale_ << std::endl;
			this->fLog << "   >----> newp:   " << this->orderInNextStep_ << std::endl;
		#endif
		...
		// Evaluate if changing the step is really computationally advantageous
		{
			// In normal conditions the step is changed only if the new step is at least 1.2 times larger than the previous
			...
			if (cpuTimeEquationSystem_ < 0.1 && cpuTimeEquationSystem_ < cpuTimeToFactorize_ / 3. && cpuTimeToFactorize_ > 0.1)
			{
				scl = 2.;
				#if LOG_MODE == 1
				this->fLog << "   >>>> case a " << std::endl;
				this->fLog << "   >>>>        " << cpuTimeEquationSystem_ << " " << cpuTimeToFactorize_ << std::endl;
				#endif
			}
			...
			if (cpuTimeEquationSystem_ < cpuTimeToFactorize_ / 100. && cpuTimeToFactorize_ > 1. && iterOfConstStepsForLargeFactorizationTime_ <= 100)
			{
				...
				#if LOG_MODE == 1
					this->fLog << "   >>>> case b " << std::endl;
					this->fLog << "   >>>>        " << cpuTimeEquationSystem_ << " " << cpuTimeToFactorize_ << " " << iterOfConstStepsForLargeFactorizationTime_ << std::endl;
				#endif
			}
			...
			if (iterOfConstStepsForLargeFactorizationTime_ >= 100)
			{
				...
				#if LOG_MODE == 1
					this->fLog << "   >>>> case c " << std::endl;
					this->fLog << "   >>>>        " << cpuTimeEquationSystem_ << " " << cpuTimeToFactorize_ << " " << iterOfConstStepsForLargeFactorizationTime_ << std::endl;
				#endif
			}

			#if LOG_MODE == 1
			this->fLog << "   >>>> scaleThreshold: " << scl << std::endl;
			this->fLog << "   >>>> hScaleMax:      " << hScaleMax_ << std::endl;
			#endif
			...
			// This means that it is very convenient to change the step (and/or the order)
			else
			{
				#if LOG_MODE == 1
				this->fLog << "   >>>> increased time step" << std::endl;
				#endif
		...
		// 1.  If more than 500 steps were done without changing the step size or
		// 2.  If 1000 steps were done and the the time step is very small 
		// ==> It is better to reduce the step (by halving it) and force the solver to use the minimum order
		if (	iterConstSteps_ > 500 || 
				(this->numberOfSteps_ % 1000 == 0 && this->numberOfSteps_ != 0 && (std::fabs(this->h_) < 1.e-7 || std::fabs(this->h_) < 1.e-5*t_))  )
		{
			if (this->p_ > this->MIN_ORDER_DAE)
			{
				fixedOrderMin_ = true;
				iterMinimumOrder_ = 0;
				this->orderInNextStep_ = this->MIN_ORDER_DAE;
				this->daeOrderStatus_ = ORDER_STATUS_DECREASED;
				this->hScale_ = 0.5;
				this->hNextStep_ = this->h_*this->hScale_;
				this->daeHStatus_ = H_STATUS_DECREASED;
			...
			#if LOG_MODE == 1
			this->fLog << "   >>>> forcing minimum order " << std::endl;
			#endif
		}
		...
		if (this->daeHStatus_ == H_STATUS_INCREASED)
			this->numberOfIncreasedSteps_++;

		if (this->daeHStatus_ == H_STATUS_DECREASED)
			this->numberOfDecreasedSteps_++;
	}
	...
	void MultiValueSolver<Method>::MultiValueStep()
	{
		...
		// Please note that the D matrix is not strictly needed, 
		// since only sums of vectors can be adopted as
		...
		this->iterConvergence_ = this->FindCorrection(t_, one_over_epsilon_, one_over_epsilon_system_);
	...
	template <typename Method>
	void MultiValueSolver<Method>::DaeSummary(std::ostream& out)
	{
		if (printResults_ == true)
		{
			out << std::endl;
			out << "DAE system solution" << std::endl;
			out << "---------------------------------------------------------------------------------------" << std::endl;
