...
#ifndef OpenSMOKE_Solve_Band_Ida_H
#define OpenSMOKE_Solve_Band_Ida_H
...
namespace DaeSMOKE
{
	...
	int Solve_Band_Ida(Object* object, const DaeSMOKE::DaeSolver_Parameters& parameters, const double t0, const double tEnd)
	{
		...
		// Memory allocation
		{
			...
			if (check_flag((void *)res, (char*)"N_VNew_Serial", 0)) return(1);
			id = N_VNew_Serial(neq);
			if (check_flag((void *)id, (char*)"N_VNew_Serial", 0)) return(1);
		...
		if (check_flag(&flag, (char*)"IDASetUserData", 1)) return(1);

		if (parameters.sparse_linear_algebra() == true)
		{
			data->J = N_VNew_Serial(neq);
			if (check_flag((void *)data->J, (char*)"N_VNew_Serial", 0)) return(1);
			data->invJ = N_VNew_Serial(neq);
			if (check_flag((void *)data->invJ, (char*)"N_VNew_Serial", 0)) return(1);
		}
		...
		// Differential/Algebraic equations
		{
			...
			if (check_flag(&flag, (char*)"IDASetId", 1)) return(1);
		}
		...
		// Assign the system
		{
			...
			if (check_flag(&flag, (char*)"IDAInit", 1)) return(1);
		}
		...
		// Set optional input
		{
			...
			if (parameters.relative_tolerances().size() == neq)
			{
				...
				if (check_flag(&flag, (char*)"IDASVtolerances", 1)) return(1);
			...
			if (check_flag(&flag, (char*)"IDASetNonlinConvCoef", 1)) return(1);

			// Maximum order
			flag = IDASetMaxOrd(mem, parameters.maximum_order());
			if (check_flag(&flag, (char*)"IDASetMaxOrd", 1)) return(1);

			// Initial step size
			if (parameters.initial_step() > 0.)
			{
			flag = IDASetInitStep(mem, parameters.initial_step());
			if (check_flag(&flag, (char*)"IDASetInitStep", 1)) return(1);
			}
		...
		// Attach band linear solver
		{
			...
			if (parameters.sparse_linear_algebra() == false)
			{
				...
				#if OPENSMOKE_USE_MKL == 1
				{
					...
					if (check_flag((void *)A, (char*)"SUNBandMatrix", 0)) return(1);

					/* Create banded SUNLinearSolver object for use by CVode */
					LS = SUNLinSol_LapackBand(y, A);
					if (check_flag((void *)LS, (char*)"SUNLinSol_LapackBand", 0)) return(1);
				}
				#else
				{
					/* Create banded SUNMatrix for use in linear solves -- since this will be factored,
					...
					if (check_flag((void *)LS, (char*)"SUNBandLinearSolver", 0)) return(1);
				}
				#endif

				/* Attach the matrix and linear solver */
				flag = IDASetLinearSolver(mem, LS, A);
				if (check_flag(&flag, (char*)"IDASetLinearSolver", 1)) return(1);
			}
			...
			else
			{
				...
				if (check_flag((void *)LS, (char*)"SUNLinSol_SPGMR", 0)) return(1);

				/* IDA recommends allowing up to 5 restarts (default is 0) */
				ier = SUNLinSol_SPGMRSetMaxRestarts(LS, 5);
				if (check_flag(&ier, (char*)"SUNLinSol_SPGMRSetMaxRestarts", 1)) return(1);

				/* Attach the linear sovler */
				ier = IDASpilsSetLinearSolver(mem, LS);
				if (check_flag(&ier, (char*)"IDASpilsSetLinearSolver", 1)) return(1);
				...
				//if (check_flag(&ier, (char*)"IDASpilsSetPreconditioner", 1)) return(1);
			}
		}
		...
		if (check_flag(&flag, (char*)"IDACalcIC", 1)) return(1);
		...
		for (unsigned int iout = 1; iout <= n_steps; iout++)
		{
			...
			if (check_flag(&flag_solver, (char*)"IDASolve", 1)) return(1);
	...
	static void IDAStatistics(void *mem, realtype cpuTime, N_Vector y, N_Vector yp)
	{
		...
		check_flag(&flag, (char*)"IDAGetNumSteps", 1);
		flag = IDAGetNumNonlinSolvIters(mem, &nni);
		flag = IDAGetLastOrder(mem, &kused);
		check_flag(&flag, (char*)"IDAGetLastOrder", 1);
		flag = IDAGetLastStep(mem, &hused);
		check_flag(&flag, (char*)"IDAGetLastStep", 1);
		flag = IDADlsGetNumJacEvals(mem, &nje);
		check_flag(&flag, (char*)"IDADlsGetNumJacEvals", 1);
		flag = IDAGetNumResEvals(mem, &nre);
		check_flag(&flag, (char*)"IDAGetNumResEvals", 1);
		flag = IDADlsGetNumResEvals(mem, &nreLS);
		check_flag(&flag, (char*)"IDADlsGetNumResEvals", 1);
		flag = IDAGetNumSteps(mem, &nst);
		check_flag(&flag, (char*)"IDAGetNumNonlinSolvIters", 1);
