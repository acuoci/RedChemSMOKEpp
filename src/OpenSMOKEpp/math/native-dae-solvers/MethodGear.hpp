...
namespace DaeSMOKE
{
	template <typename DAESystemKernel>
	const double MethodGear<DAESystemKernel>::DELTA_ALFA1 = 0.05;		//!< reduction of safety coefficient for the current order minus 1 (see Buzzi-Ferraris, eq. 29.187)
	
	template <typename DAESystemKernel>
	const double MethodGear<DAESystemKernel>::ALFA2 = 0.8;				//!< reduction of safety coefficient for the current order (see Buzzi-Ferraris, eq. 29.188)
	
	template <typename DAESystemKernel>
	const double MethodGear<DAESystemKernel>::DELTA_ALFA3 = 0.25;		//!< reduction of safety coefficient for the current order plus 1 (see Buzzi-Ferraris, eq. 29.189)


	template <typename DAESystemKernel>
	MethodGear<DAESystemKernel>::MethodGear()
	{
	}
	...
	template <typename DAESystemKernel>
	void MethodGear<DAESystemKernel>::ResetMethod()
	{
		...
		daeHStatus_ = H_STATUS_CONST;
		daeOrderStatus_ = ORDER_STATUS_CONST;
		status_ = DAE_STATUS_TO_BE_INITIALIZED;
	...
	template <typename DAESystemKernel>
	void MethodGear<DAESystemKernel>::MemoryAllocationMethod()
	{
		...
		alfa2_ = new double[MAX_ORDER_DAE + 1];
		for (unsigned int i = 0; i <= MAX_ORDER_DAE; i++)
			alfa2_[i] = ALFA2;

		// Initialize vector of error coefficients Ep (Buzzi-Ferraris, 29.204)
		Ep_ = new double[MAX_ORDER_DAE + 1];
		for (unsigned int j = 0; j <= MAX_ORDER_DAE; j++)
			Ep_[j] = Factorial(j);

		// Allocating memory for vector r
		r_ = new Eigen::VectorXd[MAX_ORDER_DAE + 1];
		for (unsigned int i = 0; i <= MAX_ORDER_DAE; i++)
		{
			r_[i].resize(MAX_ORDER_DAE);
			r_[i].setZero();
		}
		...
		// Order 5: 120/274 1 225/274 85/274 15/274 1/274
		{
			r_[0](0) = 1.;
			r_[1](0) = 1.;
			for (unsigned int j = 2; j <= MAX_ORDER_DAE; j++)
			{
		...
		z_ = new Eigen::VectorXd[MAX_ORDER_DAE + 3];
		v_ = new Eigen::VectorXd[MAX_ORDER_DAE + 3];
		for (unsigned int i = 0; i <= MAX_ORDER_DAE + 2; i++)
		{
	...
	template <typename DAESystemKernel>
	unsigned int MethodGear<DAESystemKernel>::FindCorrection(const double t, const Eigen::VectorXd& errorWeights, const Eigen::VectorXd& errorWeightsSystem)
	{
		...
		// Call the equations
		// ptrObject->jacobianIndex = 0; // TODO(?)
		this->Equations(vb_, t, f_);
		numberOfFunctionCalls_++;
		// ptrObject->jacobianIndex = -1; // TODO(?)
		...
		if (jacobianStatus_ == JACOBIAN_STATUS_HAS_TO_BE_CHANGED)
		{
			...
			switch (jacobianType_)
			{
				...
				case JACOBIAN_TYPE_NUMERICAL:
					jacobianStatus_ = JACOBIAN_STATUS_MODIFIED;
					this->NumericalJacobian(vb_, t, f_, h_, errorWeightsSystem, max_constraints_, max_values_);
					numberOfJacobians_++;
					factorizationStatus_ = MATRIX_HAS_TO_BE_FACTORIZED;
					break;
			}
		...
		// The hr0 value is used to build the G matrix (see Eq. 29.214, pag. 691)
		...
		if (factorizationStatus_ == MATRIX_HAS_TO_BE_FACTORIZED)
		{
			...
			// Build and factorize the matrix
			const bool factorization_result = this->BuildAndFactorizeMatrixG(hr0, r_[0](p_ - 1));
			
			// Check if the matrix factorization was successfull
			// TODO: we conservatile assume that in case of failure of factorization,
			//       the whole DAE solution failed. In the future we would like to do
			//       something smarter, i.e. apply some corrective measures (change the
			//       time step or the integration order and retry the factorization)
			if (factorization_result == false)
			{
		...
		// The first iteration of the Newton's method is performed
		...
		*/
		...
		for (unsigned int i = 0; i < this->ne_; i++)
		{
			if (this->equation_type_[i] == OpenSMOKE::EQUATION_TYPE_DIFFERENTIAL)
				vc_(i) = deltab_(i) = h_*f_(i) - v_[1](i);
			else
			{
				vc_(i) = deltab_(i) = f_(i);
				vc_(i) *= h_;
			}
		}

		// Solve the linear system (factorization already done)
		const bool solve_result = this->SolveLinearSystem(deltab_);

		// Check if the linear system was correctly solved
		// TODO: we conservatile assume that in case of failure of solution,
		//       the whole DAE solution failed. In the future we would like to do
		//       something smarter, i.e. apply some corrective measures (change the
		//       time step or the integration order and retry the factorization)
		...
		// Newton's iterations
		double  oldCorrectionControl = 0.;
		for (unsigned int k = 1; k <= maxConvergenceIterations_; k++)
		{
			// Estimation of the error
			const double correctionControl = r_[0](p_ - 1)*OpenSMOKE::ErrorControl(deltab_, errorWeightsSystem);
			...
			*/
			...
			{
				if (this->equation_type_[i] == OpenSMOKE::EQUATION_TYPE_DIFFERENTIAL)
					vc_(i) = deltab_(i) = -b_(i) + h_*f_(i) - v_[1](i);
				else
				{
					vc_(i) = deltab_(i) = f_(i);
					vc_(i) *= h_;
			...
			// Check if the solution was really calculated
			// TODO: we conservatile assume that in case of failure of solution,
			//       the whole DAE solution failed. In the future we would like to do
			//       something smarter, i.e. apply some corrective measures (change the
			//       time step or the integration order and retry the factorization)
	...
	template <typename DAESystemKernel>
	void MethodGear<DAESystemKernel>::WhatToDoInCaseOfConvergenceFailure(const double tInMeshPoint, double& tStabilize)
	{
		...
		daeHStatus_ = H_STATUS_DECREASED;

		#if LOG_MODE == 1
			fLog << "   > numberOfSteps: " << numberOfSteps_ << std::endl;
			fLog << "   > hScale: " << hScale_ << std::endl;
		#endif
		...
		{
			p_ -= 1;
			daeOrderStatus_ = ORDER_STATUS_DECREASED;
		}
		...
		#if LOG_MODE == 1
			fLog << "   > alfa2_[4]: " << alfa2_[4] << std::endl;
			fLog << "   > alfa2_[5]: " << alfa2_[5] << std::endl;
		#endif
		...
		if (numberOfSteps_ > 5)
		{
			...
			#if LOG_MODE == 1
				fLog << "   > numberOfConvergenceFailuresForOrderMax: " << numberOfConvergenceFailuresForOrderMax_ << std::endl;
			#endif
			...
			if (numberOfConvergenceFailuresForOrderMax_ % 5 == 0)
			{
				...
				#if LOG_MODE == 1
					fLog << "   > Reset to 1st order" << std::endl;
				#endif
			}

			// Every 10 convergence failures, ???
			if (numberOfConvergenceFailuresForOrderMax_ % 10 == 0 && tStabilize != tInMeshPoint)
			{
				...
				#if LOG_MODE == 1
					fLog << "   > Reset tStabilize" << std::endl;
				#endif
			}
		}
		...
		if (numberOfSteps_ > stepOfLastJacobian_ + MAX_ITERATIONS_JACOBIAN / 10)
		{
			jacobianStatus_ = JACOBIAN_STATUS_HAS_TO_BE_CHANGED;
			#if LOG_MODE == 1
					fLog << "   > Force Jacobian(1) " << numberOfSteps_ << " " << stepOfLastJacobian_ << " " << MAX_ITERATIONS_JACOBIAN / 10 << std::endl;
			#endif
		}
		...
		if (numberOfSteps_ != stepOfLastJacobian_ && iterConvergenceFailure_ == MAX_CONVERGENCE_FAILURE)
		{
			jacobianStatus_ = JACOBIAN_STATUS_HAS_TO_BE_CHANGED;
			#if LOG_MODE == 1
					fLog << "   > Force Jacobian(2) " << numberOfSteps_ << " " << stepOfLastJacobian_ << " " << iterConvergenceFailure_ << std::endl;
			#endif
		}
		...
		#if LOG_MODE == 1
		fLog << "   > hScale_ " << hScale_ << std::endl;
		fLog << "   > hNextStep_" << hNextStep_ << std::endl;
		#endif

		// ???
		if (tInMeshPoint + hNextStep_ == tInMeshPoint)
		{
			if (hScale_ == .25)
				hNextStep_ *= 4.;
			else
			...
			#if LOG_MODE == 1
				fLog << "   > ??? " << hNextStep_ << std::endl;
			#endif
			
			if (tStabilize != tInMeshPoint)
			{
	...
	template <typename DAESystemKernel>
	void MethodGear<DAESystemKernel>::ConvergenceRate()
	{
	...
	template <typename DAESystemKernel>
	void MethodGear<DAESystemKernel>::CheckConstraints(Eigen::VectorXd& y)
	{
		// Minimum constraints
		if (min_constraints_ == true)
		{
	...
	template <typename DAESystemKernel>
	void MethodGear<DAESystemKernel>::DaeMethodSummary(std::ostream& out)
	{
		out << std::endl;
		...
		this->DaeSolverKernelSummary(out);
	}
